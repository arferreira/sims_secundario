class AcessosController < ApplicationController
  ...
  # GET /acessos.json
  def index
    @acessos = Acesso.where("created_at >= ?", Time.zone.now.beginning_of_day)
  end
  ...
  def create
    ...
    respond_to do |format|
      if @acesso.save
        format.html { redirect_to new_acesso_path, notice: 'Movimentação registrada com sucesso.' }
  ...
  private
    ...
    def acesso_params
      params.require(:acesso).permit(:cracha, :data, :ip, :tipo)
    end
end
